...
import org.eln2.mod.misc.LRDUCubeMask
...
abstract class NodeBase {
    ...
    fun notifyNeighbor() {
        coordinate.world().notifyNeighborsOfStateChange( coordinate.getBlockPos(), coordinate.block, true)
    ...
    fun neighborBlockRead() {
        ...
        for (direction in Direction.values()) {
            val blockPos = BlockPos(coordinate.getBlockPos()).offset(direction.toEnumFacing())
    ...
    fun physicalSelfDestruction(explosionStrength: Float) {
        ...
        isDestructing = true
        if (!Eln2Config.bigExplosionsEnabled) explosionStrength = 0f
        disconnect()
        coordinate.world().setBlockToAir(coordinate.getBlockPos())
...
/*
    ...
    companion object {
        ...
        fun isBlockWrappable(block: Block, w: World, x: Int, y: Int, z: Int): Boolean {
            if (block.isReplaceable(w, BlockPos(x, y, z))) return true
        ...
        fun tryConnectTwoNode(nodeA: NodeBase, directionA: Direction, lrduA: LRDU, nodeB: NodeBase, directionB: Direction, lrduB: LRDU) {
            ...
            if (compareConnectionMask(mskA, mskB)) {
                ...
                if (nodeA.getElectricalLoad(directionA, lrduA, mskB).also { eLoad = it } != null) {
                    ...
                    if (eLoad != null && otherELoad != null) {
                        eCon = ElectricalConnection(eLoad!!, otherELoad)
                        Eln2Object.sim.addElectricalComponent(eCon)
                ...
                if (nodeA.getThermalLoad(directionA, lrduA, mskB).also { tLoad = it } != null) {
                    ...
                    if (otherTLoad != null) {
                        ...
                        Eln2Object.sim.addThermalConnection(tCon)
