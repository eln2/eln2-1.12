...
import org.eln2.sim.PhysicalConstant
...
object Utils {
    ...
    fun println(str: String?) {
        if (Eln2Object.debug) Eln2Object.logger.info(str)
    }
    ...
    fun entityLivingViewDirection(entityLiving: EntityLivingBase): Direction {
        ...
        val dirx = floor((entityLiving.rotationYaw * 4.0f / 360.0f).toDouble() + 0.5).toInt() and 3
    ...
    @JvmStatic
    fun getItemEnergie(stack: ItemStack?): Double {
        if (stack == null) return 0.0
        return burnTimeToEnergyFactor * 80000.0 / 1600 * TileEntityFurnace.getItemBurnTime(stack)
    }
...
/*
    ...
    fun getLight(w: World, e: EnumSkyBlock?, x: Int, y: Int, z: Int): Int {
        return w.getLight(BlockPos(x, y, z)) // w.getSavedLightValue(e, x, y, z)
    }
/*
    @JvmStatic
    fun notifyNeighbor(t: TileEntity) {
        ...
        val w = t.world
        var o: TileEntity?
        o = w.getTileEntity(BlockPos(x + 1, y, z))
        if (o != null && o is ITileEntitySpawnClient) (o as ITileEntitySpawnClient).tileEntityNeighborSpawn()
        o = w.getTileEntity(BlockPos(x - 1, y, z))
        if (o != null && o is ITileEntitySpawnClient) (o as ITileEntitySpawnClient).tileEntityNeighborSpawn()
        o = w.getTileEntity(BlockPos(x, y + 1, z))
        if (o != null && o is ITileEntitySpawnClient) (o as ITileEntitySpawnClient).tileEntityNeighborSpawn()
        o = w.getTileEntity(BlockPos(x, y - 1, z))
        if (o != null && o is ITileEntitySpawnClient) (o as ITileEntitySpawnClient).tileEntityNeighborSpawn()
        o = w.getTileEntity(BlockPos(x, y, z + 1))
        if (o != null && o is ITileEntitySpawnClient) (o as ITileEntitySpawnClient).tileEntityNeighborSpawn()
        o = w.getTileEntity(BlockPos(x, y, z - 1))
        if (o != null && o is ITileEntitySpawnClient) (o as ITileEntitySpawnClient).tileEntityNeighborSpawn()
    }*/
    ...
    fun playerHasMeter(entityPlayer: EntityPlayer): Boolean {
        val cur = entityPlayer.heldEquipment
        ...
        return false // TODO: When add meter
    }
/*
    @JvmStatic
    fun getRedstoneLevelAround(coord: Coordinate, side: Direction): Int {
        var side = side
        var level = coord.world().getStrongPower(coord.getBlockPos())
        if (level >= 15) return 15
        side = side.inverse
        when (side) {
            Direction.YN, Direction.YP -> {
                level = level.coerceAtLeast(coord.world().isBlockIndirectlyGettingPowered(coord.x + 1, coord.y, coord.z, side.toSideValue()))
                if (level >= 15) return 15
                level = level.coerceAtLeast(coord.world().isBlockIndirectlyGettingPowered(coord.x - 1, coord.y, coord.z, side.toSideValue()))
                if (level >= 15) return 15
                level = level.coerceAtLeast(coord.world().isBlockIndirectlyGettingPowered(coord.x, coord.y, coord.z + 1, side.toSideValue()))
                if (level >= 15) return 15
                level = level.coerceAtLeast(coord.world().isBlockIndirectlyGettingPowered(coord.x, coord.y, coord.z - 1, side.toSideValue()))
                level = level.coerceAtLeast(coord.world().isBlockIndirectlyGettingPowered(coord.x, coord.y + 1, coord.z, side.toSideValue()))
                if (level >= 15) return 15
                level = level.coerceAtLeast(coord.world().isBlockIndirectlyGettingPowered(coord.x, coord.y - 1, coord.z, side.toSideValue()))
                if (level >= 15) return 15
                level = level.coerceAtLeast(coord.world().isBlockIndirectlyGettingPowered(coord.x, coord.y, coord.z + 1, side.toSideValue()))
...
/*
    ...
    fun getItemStack(name: String, list: MutableList<ItemStack>) {
        val aitem: Iterator<*> = Item.REGISTRY.iterator()
